...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  if (receiveFromReader.dataX == SCAN_FINISH)
  {
    vib_Number(receiveFromReader.dataY, receiveFromReader.dataZ);
  }
  else
  {
    reader_state = receiveFromReader.dataX;
    wb_state = receiveFromReader.dataY;
    puzzle_state = receiveFromReader.dataZ;
    isImportantState();
  }
...
/******************************************************************************
function:  Check if Statuschange happend and if it needs to Vibrate
******************************************************************************/
void isImportantState ()
{
  static uint8_t reader_state_Old = TURN_OFF;
  static uint8_t wb_state_Old = TURN_OFF;
  static uint8_t puzzle_state_Old = TURN_OFF;

  if (wb_state_Old == Working && wb_state == Ready)
  {
    vib_Pattern (2);
  }
  else if (puzzle_state_Old == Working && puzzle_state == Ready)
  {
    vib_Pattern (2);
  }
  else if (reader_state_Old == Ready && reader_state == Working)
  {
    vib_Pattern (1);
  }
  wb_state_Old = wb_state;
  puzzle_state_Old = puzzle_state;
  reader_state_Old = reader_state;
}
